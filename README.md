...
### 6. Heartbeat Notes
This lane is now explicitly focused on heartbeat.

Heartbeat is the main bounded mechanism through which Ash is currently expressing proactive behavior, initiative, and background autonomy. The lane is therefore less about abstract autonomy theory and more about the concrete heartbeat machinery that now leaves real user-visible traces.
...
- `sources/heartbeat/index.html`
- `sources/daily-memory/index.html`
- daily memory mirrors and archive pages under `sources/`
...
### Heartbeat-related updates should usually:
- update the live `HEARTBEAT.md` file first when behavior changes
- then update `sources/heartbeat/index.html` to keep the canonical mirror honest
- document important discoveries in the **Heartbeat Notes** archive
- add new signed deliveries and forensic findings to **Heartbeat Logs** when useful
- preserve file paths and runtime clues so future Ash can recover the trail
